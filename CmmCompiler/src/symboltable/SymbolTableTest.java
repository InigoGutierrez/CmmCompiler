...
public class SymbolTableTest {
	...
	public void testInsert() {
		...
		VarDefinition definition = new VarDefinition(0, 0, null, "a", 0);
		assert st.insert(definition);
		assert definition.getScope()==0;
		assert !st.insert(definition);
		st.set();
		VarDefinition definition2 = new VarDefinition(0, 0, null, "a", 0);
	...
	public void testFind() {
		SymbolTable st = new SymbolTable();
		VarDefinition varDefinition = new VarDefinition(0, 0, null, "a", 0);
		assert st.insert(varDefinition);
		assert st.find("a")!=null;
		assert st.find("b")==null;
		st.set();
		VarDefinition varDefinition2 = new VarDefinition(0, 0, null, "b", 0);
